...
이 문서는 `awesome-llm-benchmarks` 저장소 내의 모든 벤치마크 수치(총 17개 지표)의 명확한 출처를 기록하고 추적하기 위한 파일입니다. 데이터 무결성을 위해 성능 수치가 업데이트될 때마다 이 파일도 함께 업데이트해야 합니다.
...
- **공식 기술 보고서 출처 (Primary Source)**:
  - **에이전트 및 코딩**: GAIA, OSWorld, WebArena, Tau²-Bench, BFCL, LiveCodeBench, HumanEval
  - **지식 및 추론**: GPQA Diamond, HLE, MMLU, MMLU-Pro
  - **수학 및 비전**: MATH, GSM8K, MMMU, MathVista
